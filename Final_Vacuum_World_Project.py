...
def Advanced_Agent(x,y):
    if check_size(x,y):
        ...
        count = len(optimal)
        print('Count of optimal path actions',count)
        Env_2D = VacuumEnvironments(x + 1, y + 1)
        Advanced_Vacuum_Agent = cleaner(Advanced_Agent_Program())
        Env_2D.add_thing(Advanced_Vacuum_Agent, (1, 1))
        dirt = Dirt()
        dirt2 = Dirt()
        Env_2D.add_thing(dirt, (1, 1))
        Env_2D.add_thing(dirt2, (2, 2))
        TraceAgent(Advanced_Vacuum_Agent)
        Env_2D.run(count)

        print('.........', Env_2D.percept(Advanced_Vacuum_Agent))
        print(Advanced_Vacuum_Agent.performance)
...
def Random_Book_Agent(x,y):
    psource(VacuumEnvironments)
    Env_2D = VacuumEnvironments(x+1 , y+1)
    Random_Vacuum_Agent = cleaner(program())
    Env_2D.add_thing(Random_Vacuum_Agent,[1,1])
    dirt = Dirt()
    dirt2 = Dirt()
    Env_2D.add_thing(dirt,(1,1))
    Env_2D.add_thing(dirt2,[2,2])
    ...
    Env_2D.run(20)
    TraceAgent(Random_Vacuum_Agent)
    print('.........', Env_2D.percept(Random_Vacuum_Agent))
    print(Random_Vacuum_Agent.performance)
...
if check_size(x,y):
    ...
    TraceAgent(Random_Vacuum_Agent)
    print('.........', Env_2Dprime.percept(Random_Vacuum_Agent))
    print(Random_Vacuum_Agent.performance)
